...
const Navbar = ({ss,setSs,sl,setSl}) => {
  ...
  return (
    ...
    <div className="w-full fixed bg-white dark:dark:bg-trueGray-900 z-10 top-0 ">
      <nav className=" flex-wrap  nav__item nav__item--right container ch relative sm:flex-wrap flex xl:flex-nowrap md:flex-wrap lg:flex-wrap items-center justify-between p-4 mx-auto lg:justify-between xl:px-0">
        
        {/* Logo  */}
        <Disclosure>
          {({ open }) => (
            <>
              <div className="flex flex-wrap md:w-full  items-center justify-between w-full lg:w-auto">
                <Link href="/">
                      ...
                      <Link key={index} href={item.href} className="w-full px-4 py-2 -ml-4 text-gray-500 rounded-md dark:text-gray-300 hover:text-indigo-500 focus:text-indigo-500 focus:bg-indigo-100 dark:focus:bg-gray-800 focus:outline-none">
                       ...
                       <div className="w-full">
                         {!subscriptionStatus && <SubButton style={ {width:'100%', height: '100%'}} plan={'paid'}/>}
                          {subscriptionStatus && <SubButton style={ {width:'100%', height: '100%'}} text="Manage Subscription" send="toPortal"/>}
                       </div>
                  </>
                </Disclosure.Panel>
              </div>
        ...
        <div className="hidden text-center lg:flex xlg:pl-20 .g:items-center">
          <ul className="items-center justify-end flex-1 pt-6 list-none lg:pt-0 lg:flex">
            {navigation.map((menu, index) => (
              <li className=" 2xl:pr-8 pr-0 nav__item" key={index}>
                <Link href={menu.href} className="inline-block px-4 py-2 text-lg font-normal text-gray-800 no-underline rounded-md dark:text-gray-200 dark:hover:text-orange-300 hover:text-indigo-500 focus:text-indigo-500 focus:bg-indigo-100 focus:outline-none dark:focus:bg-gray-800">
                    {menu.text}
                </Link>
              </li>
            ))}
        ...
        <div className="flex justify-center items-center ">
          ...
          <div className=" flex md:w-full sm:w-full w-full lg:w-auto  mr-4 items-center justify-between ">
            <button className=" pr-10  mr-2  rounded pl-2 dark:hover:text-orange-300 "
